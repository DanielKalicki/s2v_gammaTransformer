...
class MultiHeadSelfAttention:
    ...
    def __init__(self, n_state: int, n_head: int, attention_dropout: float,
        ...
        # self.c_attn_proj = Dense(n_state*4, use_bias=False,
        #                          name='layer_{}/c_attn_proj'.format(layer_id))
        self.activation = Gelu(accurate=True,
                               name='layer_{}/gelu2'.format(layer_id))
        self.c2_attn_proj = Dense(n_state, use_bias=False,
                                  name='layer_{}/c2_attn_proj'
                                       .format(layer_id))
    ...
    def __call__(self, x, mask):
        ...
        output = self.c2_attn_proj(output)
        return output
...
def create_gated_transformer(embedding_dim: int = 768, max_len: int = 512,
    ...
    for i in range(num_layers):
        x = GatedEncoderLayer(embedding_dim, num_heads, d_hid,
                              ...
                              accurate_gelu, gate_type)(x, attn_mask)
    if use_attn_mask:
        inputs.append(attn_mask)
